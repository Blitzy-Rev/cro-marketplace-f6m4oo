...
variable "secondary_aws_region" {
  description = "AWS region for disaster recovery and cross-region replication"
  type        = string
  default     = "us-west-2"
}
...
variable "compute" {
  description = "Compute configuration for ECS clusters, services, and tasks"
  type = object({
  ...
  default = {
    ecs_cluster_name = "molecule-platform-cluster"
    frontend_service = {
      name                 = "frontend"
      container_port       = 80
      host_port            = 80
    ...
    load_balancer_config = {
      ...
      access_logs_bucket     = "molecule-platform-logs"
...
variable "database" {
  description = "Database configuration for RDS PostgreSQL and ElastiCache Redis"
  type = object({
...
variable "storage" {
  description = "Storage configuration for S3 buckets and lifecycle policies"
  type = object({
  ...
  default = {
    document_bucket_name     = "molecule-platform-documents"
    csv_bucket_name          = "molecule-platform-csv"
    results_bucket_name      = "molecule-platform-results"
    backup_bucket_name       = "molecule-platform-backups"
...
variable "security" {
  description = "Security configuration for WAF, GuardDuty, CloudTrail, and KMS"
  type = object({
    enable_waf                = bool
  ...
  default = {
    ...
    cognito_user_pool_name    = "molecule-platform-users"
    cognito_identity_pool_name = "molecule-platform-identity"
  }
}
...
variable "monitoring" {
  description = "Monitoring configuration for CloudWatch, alarms, and dashboards"
  type = object({
