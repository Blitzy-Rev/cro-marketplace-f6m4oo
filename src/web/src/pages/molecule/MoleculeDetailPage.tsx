...
import { useParams, useNavigate, generatePath } from 'react-router-dom'; // version ^6.0.0
...
import MoleculeViewer from '../../components/molecule/MoleculeDetail';
...
export default function MoleculeDetailPage() {
  // Extract molecule ID from URL parameters using useParams hook
  ...
  const handleSubmitToCRO = useCallback(() => {
    if (molecule) {
      const submissionCreatePath = generatePath(ROUTES.SUBMISSIONS.CREATE, {});
      navigate(`${submissionCreatePath}?moleculeId=${molecule.id}`);
    }
  }, [molecule, navigate]);
  ...
  // Create a title for the page using the molecule ID
  const pageTitle = `Molecule Detail: ${molecule?.id}`;

  // Render the DashboardLayout component with the title
  return (
    <DashboardLayout title={pageTitle} loading={loading}>
